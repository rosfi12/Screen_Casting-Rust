pub mod decoder{
    ...
    // receives a byte vector that is a compressed video
    // decodes these values and outputs the decoded image and the duration of this operation
    pub fn decode(encoded_frames: Vec<u8>, width: u32, height: u32) -> (std::time::Duration, ImageBuffer<Rgba<u8>, Vec<u8>>) {
        let mut decoder = Decoder::new().expect("Not create decoder");
    
        let start_decode = Instant::now();
        
        let decoded_frame = decoder.decode(&encoded_frames).expect("Not decode frame");
        ...
        let out_img = ImageBuffer::<Rgba<u8>, _>::from_raw(width, height,  rgba_buffer)
            .expect("Not create image buffer");

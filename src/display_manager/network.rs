...
// TcpListener accepts input connections
// TcpStream for communication with clients for send/receive dates
// SocketAdrr manages IP addresses and port
use std::net::{TcpListener, TcpStream, SocketAddr, Shutdown};
use std::sync::{Arc, Mutex};
use std::thread;
use std::collections::HashMap;
// serde allow to convert Rust's structures in network's one
// bincode serialize date in binary format for an efficient trasmission
...
pub mod net {
    ...
    use crate::{enums::StreamingState, display_manager::screen::ScreenState};

    use super::*;

    // divide dates in chunks for improving efficiency
    ...
    pub struct Screenshot {
        ...
        pub height: u32,
        pub state: State,
        pub line_overlay: Option<Vec<(f32, f32, f32, f32)>>,
        pub circle_overlay: Option<Vec<(f32, f32, f32, f32)>>,
        pub text_overlay: Option<Vec<(f32, f32, String)>>,
    ...
    impl Server {
        ...
        pub fn bind_to_ip(&self, state: Arc<ScreenState>) -> io::Result<()> {
            let connection_listener = TcpListener::bind(&self.ipaddress)?;

            let clients = self.clients.clone();
            //connection_listener.set_nonblocking(true)?;
            // initialize a thread to manage incoming connections
            thread::spawn(move || {
                //println!("connection_listener created {:?}",connection_listener.local_addr());
                for stream in connection_listener.incoming() {
                    thread::sleep(Duration::from_millis(50));
                    if state.get_kill_connection_listener()==true{
                        state.set_kill_connection_listener(false);
                        //println!("drop connection_listener");
        ...
        pub fn send_to_all_clients(&self, screenshot: &Screenshot) -> io::Result<()> {
            ...
            let mut clients: std::sync::MutexGuard<'_, HashMap<SocketAddr, TcpStream>> = self.clients.lock().unwrap();
    ...
    impl Client {
        pub fn new(host_server_address: String) -> Self {
            Client {
                host_server_address,
            }
        }
        ...
        pub fn connect_to_ip(&self) -> io::Result<TcpStream> {
            let server_address = self.host_server_address.to_string();

            if let Ok(stream) = TcpStream::connect(&server_address){
                //println!("Connected to host_server at {}", server_address);
                Ok(stream)
            }
            else{
                //println!("Not connected to host_server");
        ...
        // receive an image from server and deserialize it in Screenshot structure
            ...
            loop {
                ...
                match stream.read(&mut buffer) {
                    ...
                    Err(ref e) if e.kind() == io::ErrorKind::WouldBlock || e.kind() == io::ErrorKind::TimedOut => {
                        ...
                        state.set_reconnection_flag(true);
                        break;
                    }
                    Err(e) => {

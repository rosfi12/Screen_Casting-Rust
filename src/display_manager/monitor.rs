pub mod capture{
    ...
    // infinite loop to obtaine screen's frame and manage streaming state
                    ...
                    Frame::BGRx(frame) => {
                        ...
                        match state.get_sc_state(){
                            ...
                            StreamingState::PAUSE => {
                                let _unused =state.cv.wait_while(state.broadcast_state.lock().unwrap(), |s| *s!=StreamingState::START);
                            },
